...
def get_retriever(retriever_type="BM25", top_k=1):
    ...
    if retriever_type == "BM25":
        ...
        retriever = BM25Retriever(document_store=doc_store, top_k=top_k)
    elif retriever_type == "TFIDF":
        # Not recommended as BM25 is an improved version of tfidf
        retriever = TfidfRetriever(document_store=doc_store, top_k=top_k)
    elif retriever_type == "Embedding-deepset-sentence-bert":
        # Recommended, but requires a sentence similarity model
        retriever = EmbeddingRetriever(document_store=doc_store, embedding_model="deepset/sentence_bert", use_gpu=True, top_k=top_k)
    elif retriever_type == "DPR":
        # Computationally expensive, needs two models.
        retriever =  DensePassageRetriever(document_store=doc_store, query_embedding_model="facebook/dpr-question_encoder-single-nq-base", passage_embedding_model="facebook/dpr-ctx_encoder-single-nq-base", top_k=top_k)
    return retriever
